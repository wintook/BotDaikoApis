...
const getArticulos = async (req, res) => {
  const dataArticulos = await ArticulosModel.getArticulos(req);
  if (dataArticulos) {
...
const getArticulo = async (req, res) => {
  const dataArticulo = await ArticulosModel.getArticulo(req);
  if (dataArticulo) {
    res.status(200).send(dataArticulo);
  } else {
    res.status(404).send("404 Not Found");
  }
};
...
const getOrders = async (req, res) => {
  ...
  for (let i = 0; i < pedidos.length; i++) {
    ...
    //   return res.status(400).send({
    //     error: true,
    //     message: `Falta el ID del cliente en el pedido ${i + 1}.`,
    //   });
    // } else if (!articulo_id) {
    if (!articulo_id) {
      return res.status(400).send({
  ...
  try {
    // Llamar a la función en el modelo para crear los pedidos
    // const createdOrders = await ArticulosModel.getOrders(id, pedidos);
...
const addProducts = async (req, res) => {
  ...
  for (const producto of body.productos) {
    ...
    const results = await ArticulosModel.addProducts(req);
    console.log("results existe controlador:", results);
  console.log("results controlador: ", results)
  const errorExists = results.error;
  console.log("errorExists", errorExists)
...
//     res.status(400).send({
//       error: true,
//       message: error.message || "Error al agregar productos al carrito.",
//     });
//   }
 };
